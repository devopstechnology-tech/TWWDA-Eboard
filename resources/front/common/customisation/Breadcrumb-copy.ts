...
export  function formatCustomDuration(duration: string) {
...
export function formattedDate(isoDate: string) {
    ...
    try {
        const formatted = formatInTimeZone(isoDate, KENYA_TIMEZONE, formatString);
        return formatted;
    } catch (error) {
...
export function formattedDateTime(isoDate: string) {
...
export function formatTime(datetimeStr: string): string {
    ...
    const options: Intl.DateTimeFormatOptions = {
        ...
        minute: 'numeric',
        hour12: true,
        timeZone: 'Africa/Nairobi', // Set to Kenyan timezone
    };
...
export function formatDuration(startDate: string, startTime: string,  endTime: string): string {
...
function getDayWithSuffix(day: number): string {
    if (day > 3 && day < 21) return `${day}th`; // because most suffixes are "th"
    switch (day % 10) {
        case 1: return `${day}st`;
        case 2: return `${day}nd`;
        case 3: return `${day}rd`;
