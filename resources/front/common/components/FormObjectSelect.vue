...
const props = withDefaults(defineProps<Props>(), {
    // options: [] as MailType[], // Ensure options is always an array
    direction: 'right',
    crud: false,
});

const {errorMessage, value} = useField<MailType>(props.name);
const emit = defineEmits(['addNew', 'selectedItem']);

// Handling default selection
const defaultMailType = computed(() => props.options.length > 0 ? props.options[0] : null);
watch(defaultMailType, (newVal) => {
    if (newVal) {
        value.value = newVal;
    }
}, {immediate: true});
