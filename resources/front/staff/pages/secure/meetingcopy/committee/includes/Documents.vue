    ...
    try {
        if (action.value === 'create') {
            if(type.value === 'folder'){
                ...
                await useCreateMeetingFolderRequest(payload, meetingId, boardId);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                await useCreateMeetingUploadFolderRequest(payload, meetingId, boardId);
            }
        } else {
            if(type.value === 'folder'){
                const payload: FolderRequestPayload = {
                ...
                await useUpdateMeetingFolderRequest(payload, meetingId, boardId);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                const folder_id = folderId.value ? folderId.value.toString() : null;
                payload.folder_id = folder_id;
                await useUpdateMeetingUploadFolderRequest(payload, meetingId, boardId);
            }
        }
        await fetchMeetingFolders();
        resetFolder();
