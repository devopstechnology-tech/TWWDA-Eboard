...
import {useGetBoardMembersRequest, useGetMembersRequest} from '@/common/api/requests/modules/member/useMemberRequest';
import {
    useCreateBoardPollRequest,
    useGetBoardPollsRequest,
    useUpdateBoardPollRequest,
...
const onSubmit = handleSubmit(async (values, {resetForm}) => {
    try {
        ...
        if (action.value === 'create') {
            await useCreateBoardPollRequest(payload, boardId);
        } else {
            const poll_id = pollId.value as string;
            await useUpdateBoardPollRequest(payload, poll_id);
        }
        PollModal.value?.close();
        await fetchBoardPolls();
        reset();
    } catch (err) {
...
const getBoardPolls = () => {
    return useQuery({
        queryKey: ['getBoardPollsKey', boardId],
        queryFn: async () => {
            const response = await useGetBoardPollsRequest(boardId, {paginate: 'false'});
            return response.data;
        },
    });
};
const {isLoading:isLoadingBoardPolls, data: Polls, refetch: fetchBoardPolls} = getBoardPolls();
onMounted(async () => {
    // window.dispatchEvent(new CustomEvent('updateTitle', {detail: 'Board Tasks'}));
    fetchMembers();
    fetchBoardPolls();
});
    ...
    <div v-if="authStore.hasPermission(['view poll'])">
        <div class="card h-full">
            ...
            <div class="card-container">
                <div v-if="isLoadingBoardPolls">Loading Board Polls...</div>
                <div v-else>
