...
import {
    useCreateMeetingFolderRequest,
    useCreateMeetingUploadFolderRequest,
    useGetMeetingFoldersRequest,
    useUpdateMeetingFolderRequest,
    useUpdateMeetingUploadFolderRequest,
...
const getMeetingFolders = () => {
    return useQuery({
        queryKey: ['getMeetingFoldersKey', meetingId],
        queryFn: async () => {
            const response = await useGetMeetingFoldersRequest(meetingId, {paginate: 'false'});
            return response.data[0].children;
        },
    });
};

const {isLoading, data: Folders, refetch: fetchMeetingFolders} = getMeetingFolders();

onMounted(async () => {
    await fetchMeetingFolders();
});
//
...
watch(Folders, (newFolders) => {
    if (newFolders && newFolders.length > 0) {
        if (navigationPath.value.length === 1) {
            // console.log('newFolders', newFolders, navigationPath.value.length === 1);
            const firstFolder = newFolders[0];
            // console.log('navigationPath.value', navigationPath.value);
            // console.log('firstFolder.folderable && firstFolder.folderable.details', firstFolder.folderable && firstFolder.folderable.details);
            // if (firstFolder.folderable && firstFolder.folderable.details) {
            navigationPath.value[0].id = firstFolder.folderable_id.toString();
    ...
    try {
        if (action.value === 'create') {
            if(type.value === 'folder'){
                ...
                await useCreateMeetingFolderRequest(payload, meetingId);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                await useCreateMeetingUploadFolderRequest(payload, meetingId);
            }
        } else {
            if(type.value === 'folder'){
                const payload: FolderRequestPayload = {
                    ...
                    icon: null,
                    id: null,
                    meeting_id: values.meeting_id.toString(),
                    name: values.name,
                    parent_id: values.parent_id.toString(),
                    type: values.type,
                    fileupload:null,
                ...
                await useUpdateMeetingFolderRequest(payload, meetingId);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                const folder_id = folderId.value ? folderId.value.toString() : null;
                payload.folder_id = folder_id;
                await useUpdateMeetingUploadFolderRequest(payload, meetingId);
            }
        }
        await fetchMeetingFolders();
        resetFolder();
                            ...
                            <form novalidate @submit.prevent="onSubmit"
                                ...
                                <div  class="row">
                                    ...
                                    <div v-if="type === 'file'" class="row mt-2 ml-2">
                                        <FormUpload
                                            :labeled="true"
                                            label="Upload Cover Image for the Committee"
                                            name="fileupload"
                                            class="w-full text-sm  text-white tracking-wide col-md-12"
                                            placeholder="Enter Cover Image of the Committee"
                                            type="file"
