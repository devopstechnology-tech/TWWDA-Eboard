...
import {useGetSingleCommitteeMeetingRequest} from '@/common/api/requests/modules/meeting/useMeetingRequest'; // Import your API function
...
const fallbackCopyTextToClipcommittee = (text) => {
...
// Usage
const copyLinkToClipcommittee = async (link) => {
    if (!navigator.clipcommittee) {
        // Fallback method
        fallbackCopyTextToClipcommittee(link);
        alert('Link copied to clipcommittee, paste it on your browser or share it to your platform');
        return;
    }

    try {
        await navigator.clipcommittee.writeText(link);
        alert('Link copied to clipcommittee!');
    } catch (err) {
...
const getMeeting = () => {
    return useQuery({
        queryKey: ['getMeetingKey', committeeId, meetingId],
        queryFn: async () => {
            const response = await useGetSingleCommitteeMeetingRequest(meetingId);
    ...
    <div class="card" v-if="Meeting">
        ...
        <div class="card-body">
            <div class="space-y-6">
                ...
                <div class="flex" >
                    ...
                    <div>
                        <div class="flex mb-2 flex-wrap" v-if="Meeting?.link">
                            ...
                            <!-- Copy Link Button -->
                            <button type="button" @click="copyLinkToClipcommittee(Meeting?.link)"
                ...
                <div>
                    <!-- <a href="https://app.committeeable.com/felix-1/groups/dff155-committee"
                        class="badge badge-soft">{{  meeting?.committee?.title }}  Committee</a>
