...
import {
    useCreateCommitteePollRequest,
    useGetCommitteePollsRequest,
    useUpdateCommitteePollRequest,
...
import {Member, SelectedResult} from '@/common/parsers/memberParser';
...
const selectedMemberIds = ref<PollAssignee[]>([]);
...
function mappollassigneesToReactiveRef(pollassignees: PollAssignee[]) {
    selectedMemberIds.value = pollassignees.map(pollassignee => ({
        id: pollassignee.id,// Default value as empty string
        full_name: pollassignee.user.full_name,
        poll_id: pollassignee.poll_id,
        member_id: pollassignee.member_id,
        user: pollassignee.user,
    }));
    setFieldValue('pollassignees', selectedMemberIds.value);
}
...
const onSubmit = handleSubmit(async (values, {resetForm}) => {
    try {
        ...
        if (action.value === 'create') {
            await useCreateCommitteePollRequest(payload, committeeId);
        } else {
            const poll_id = pollId.value as string;
            await useUpdateCommitteePollRequest(payload, poll_id);
        }
        PollModal.value?.close();
        await fetchCommitteePolls();
        reset();
    } catch (err) {
...
const getCommitteePolls = () => {
    return useQuery({
        queryKey: ['getCommitteePollsKey', committeeId],
        queryFn: async () => {
            const response = await useGetCommitteePollsRequest(committeeId, {paginate: 'false'});
            return response.data;
        },
    });
};
const {isLoading:isLoadingCommitteePolls, data: Polls, refetch: fetchCommitteePolls} = getCommitteePolls();
onMounted(async () => {
    // window.dispatchEvent(new CustomEvent('updateTitle', {detail: 'Committee Tasks'}));
    fetchMembers();
    fetchCommitteePolls();
});
    ...
    <div v-if="authStore.hasPermission(['view poll'])">
        <div class="card h-full">
            ...
            <div class="card-container">
                <div v-if="isLoadingCommitteePolls">Loading Committee Polls...</div>
                <div v-else>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-2 gap-4 p-2">
                            ...
                            <div class="col-span-2">
                                <form novalidate @submit.prevent="onSubmit" class="">
                                    ...
                                    <div v-if="selectedMemberIds && selectedMemberIds.length || 
