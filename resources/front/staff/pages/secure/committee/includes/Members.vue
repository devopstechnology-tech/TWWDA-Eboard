...
import {
  useCreateCommitteeMemberRequest,
    useCreateMemberRequest,
    useGetCommitteeMembersRequest,
    useUpdateCommitteeMemberPositionRequest,
    useUpdateCommitteeMemberRequest,
    useUpdateMemberPositionRequest,
    useUpdateMemberRequest,
} from '@/common/api/requests/modules/member/useMemberRequest';
import {useGetCommitteePositionsRequest} from '@/common/api/requests/modules/member/usePositionRequest';
...
import MemberPosition from '@/staff/pages/secure/committee/includes/MemberPosition.vue';
...
const memberschema = yup.object({
    ...
    committee_id: yup.mixed().required(),
...
const onSubmit = handleSubmit(async (values, {resetForm}) => {
    try {
        ...
        if (action.value === 'create') {
            await useCreateCommitteeMemberRequest(payload, committeeId);
        } else if(action.value === 'edit') {
            await useUpdateCommitteeMemberRequest(payload, committeeId);
        } else if(action.value === 'position') {
            const payload: MemberPositionRequestPayload = {
                id: values.id,
                position_id: values.position_id,
            };
            await useUpdateCommitteeMemberPositionRequest(payload, committeeId);
        }
...
const getCommitteeMembers = () => {
    return useQuery({
        queryKey: ['getCommitteeMembersKey', committeeId],
        queryFn: async () => {
            const response = await useGetCommitteeMembersRequest(committeeId, {paginate: 'false'});
            // console.log('response.data', response.data);
            return response.data;
        },
    });
};
const {isLoading, data: Members, refetch: fetchCommitteeMembers} = getCommitteeMembers();

// Fetch positions once and pass to child components
const allPositions = ref<Position[]>([]);
const getPositions = async () => {
    const data = await useGetCommitteePositionsRequest({paginate: 'false'});
...
<template>
    <div class="card">
        <div class="card-header flex items-center">
            <h2 class="card-header-title h3">Committee Members</h2>
                        ...
                        <div class="card-body box-profile">
                            <div class="text-center">
                                <h3 class="profile-username text-center text-primary">
                                    ...
                                    <button v-if="authStore.hasPermission(['view committee member profile'])"
                            ...
                            <!-- Action Column -->
                            <div class="mt-3" v-if="authStore.hasPermission(['assign committee position'])">
