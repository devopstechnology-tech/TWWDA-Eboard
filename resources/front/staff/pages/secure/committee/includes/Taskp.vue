...
const onSubmit = handleSubmit(async (values, {resetForm}) => {
    try {
        ...
        if (action.value === 'create') {
            await useCreateCommitteeTaskRequest(payload, committeeId);
        } else {
            const task_id = taskId.value as string;
            await useUpdateCommitteeTaskRequest(payload, task_id);
        }
        TaskModal.value?.close();
        await fetchCommitteeTasks();
        reset();
    } catch (err) {
...
const getMembers = () => {
    return useQuery({
        queryKey: ['getCommitteeMembersKey'],
        queryFn: async () => {
...
const getCommitteeTasks = () => {
    return useQuery({
        queryKey: ['getCommitteeTasksKey', committeeId],
        queryFn: async () => {
            const response = await useGetCommitteeTasksRequest(committeeId, {paginate: 'false'});
            return response.data;
        },
    });
};

const {isLoading: isLoadingCommitteeTasks, data: Tasks, refetch: fetchCommitteeTasks} = getCommitteeTasks();
