...
import {
    useCreateCommitteeFolderRequest,
    useCreateCommitteeUploadFolderRequest,
    useGetCommitteeFoldersRequest, 
    useUpdateCommitteeFolderRequest, 
    useUpdateCommitteeUploadFolderRequest,
...
const getCommitteeFolders = () => {
    return useQuery({
        queryKey: ['getCommitteeFoldersKey', committeeId],
        queryFn: async () => {
            const response = await useGetCommitteeFoldersRequest(committeeId, {paginate: 'false'});
            return response.data;
        },
    });
};

const {isLoading, data: Folders, refetch: fetchCommitteeFolders} = getCommitteeFolders();

onMounted(async () => {
    await fetchCommitteeFolders();
});
//
...
const {
    ...
    initialValues: {
        name: '',
        meeting_id: committeeId,//just default though used
    ...
    try {
        if (action.value === 'create') {
            if(type.value === 'folder'){
                ...
                await useCreateCommitteeFolderRequest(payload, committeeId);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                await useCreateCommitteeUploadFolderRequest(payload, committeeId);
            }
        } else {
            if(type.value === 'folder'){
                const payload: FolderRequestPayload = {
                ...
                await useUpdateCommitteeFolderRequest(payload, committeeId, payload.folder_id);
            }else{
                const payload: FolderRequestPayload = {
                    children: [],
                    created_at: null,
                ...
                const folder_id = folderId.value ? folderId.value.toString() : null;
                payload.folder_id = folder_id;
                await useUpdateCommitteeUploadFolderRequest(payload, committeeId, payload.folder_id);
            }
        }
        await fetchCommitteeFolders();
        resetFolder();
                                ...
                                <td class="w-28">
                                    <div v-if="folder.type==='folder'">
                                        <div  
                                            ...
                                            <img :src="loadDefaultFolderSvg(folder.icon)"
